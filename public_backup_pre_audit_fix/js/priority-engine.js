...
(function (root, factory) {
  ...
  function calculateHazardSeverity(incident) {
    ...
    return 60; // Baseline fallback so it is never completely blank
  }
  ...
  function calculatePopulationRisk(incident) {
    ...
    if (rawPop !== null && rawPop !== undefined && !isNaN(Number(rawPop))) {
      ...
      // Fallback for incident without explicit population (e.g., standard citizen report)
      pop = 55; 
    }
  ...
  function calculateVulnerability(incident) {
    ...
    if (!hasData) return 50; // Fallback so it is never blank
  ...
  function calculateImmediateLifeRisk(incident) {
    ...
    return 30; // Fallback so it is never blank
  }
  ...
  function calculateResponseUrgency(incident) {
    ...
    return 50; // Fallback so it is never blank
  }
