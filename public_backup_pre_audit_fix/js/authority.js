...
function switchView(viewKey) {
  ...
  const titleMap = {
    ...
    'safesites':       'Eligible Evacuation Shelters & Carrying Capacity',
...
function openZoneInfoPanel(zone, lat, lng) {
  ...
  if (rawRisk.includes('RED') || rawRisk === 'CRITICAL') {
    riskBadgeText = 'RED / CRITICAL';
    badgeBg = 'rgba(239, 68, 68, 0.15)';
    badgeColor = '#dc2626';
  } else if (rawRisk.includes('ORANGE') || rawRisk === 'HIGH') {
    riskBadgeText = 'ORANGE / HIGH RISK';
    badgeBg = 'rgba(249, 115, 22, 0.15)';
    badgeColor = '#ea580c';
  } else if (rawRisk.includes('YELLOW') || rawRisk === 'MODERATE') {
    riskBadgeText = 'YELLOW / MODERATE';
    badgeBg = 'rgba(234, 179, 8, 0.18)';
    badgeColor = '#a16207';
  }
  ...
  // Click on empty map closes panel
  const mapInst = authMapInstance || window.authMapInstance;
  if (mapInst && mapInst.getMap()) {
    mapInst.getMap().once('click', () => {
      panel.style.display = 'none';
    });
  }

  // Fetch telemetry via Open-Meteo
  fetch(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lng}&current=temperature_2m,relative_humidity_2m,apparent_temperature,precipitation,rain,showers,weather_code,surface_pressure,wind_speed_10m,wind_direction_10m,wind_gusts_10m`)
    .then(r => r.json())
    .then(data => {
      ...
      document.getElementById('zip-source').textContent = 'Open-Meteo Live';
      document.getElementById('zip-time').textContent = new Date().toLocaleTimeString();
        ...
        return `
          <div style="margin-bottom:6px;">
            <div style="display:flex; justify-content:space-between; font-size:11px; margin-bottom:2px;">
              ...
              <span style="font-weight:600; color:${isUnavail ? '#94a3b8' : '#0f172a'};">${displayVal}${isUnavail?'':'/100'}</span>
            </div>
    ...
    card.innerHTML = `
      ...
      <div class="report-actions">
        <div style="position:relative; display:inline-block; vertical-align:middle;">
          ...
          <div id="popover-${rep.id}" class="zone-popover" style="display:none; position:absolute; bottom:110%; left:0; background:#fff; border:1px solid #cbd5e1; border-radius:6px; box-shadow:0 10px 15px -3px rgba(0,0,0,0.1); z-index:1000; width:170px; overflow:hidden; text-align:left;">
            ...
            <div style="padding:8px 12px; cursor:pointer; color:#22c55e; font-weight:600; font-size:12px; transition:background 0.2s;" onmouseover="this.style.background='#f0fdf4'" onmouseout="this.style.background='transparent'" onclick="handleVerifyAllocate('${rep.id}', 'GREEN')">🟢 Safe (Green)</div>
          </div>
        </div>
  ...
  habitations.forEach((h, idx) => {
    ...
    card.innerHTML = `
      ...
      <div class="priority-action-row">
        <button type="button" class="priority-action-btn" onclick="const d = priorityExplanationData['${explainId}']; inspectEntity({name:'${safeName}', tier:'${safeTier}', lat:${hLat !== null ? hLat : 'null'}, lng:${hLng !== null ? hLng : 'null'}, population:${h.population || 0}, habitations:'District: ${safeDistrict} &bull; Recommended: ${safeAction}', factors: d.factors, score: d.score, provenance: d.provenance}, event)" aria-label="Inspect ${safeName}">
