...
(async () => {
  ...
  await page.goto('http://localhost:3001/'); await page.evaluate(() => { sessionStorage.setItem('rzi_authority_officer', JSON.stringify({ name: 'Commander', role: 'admin', clearance: 'TOP_SECRET', officerId: 'ADM-901' })); });
console.log('Navigating to authority portal...');
  ...
  await page.waitForSelector('.priority-incident-card', { timeout: 15000 }).catch(() => console.log('Timeout waiting for cards'));

  const cards = await await page.locator('.priority-incident-card').all();
